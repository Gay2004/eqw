        ...
        for (size_t column = 0; column < level.columns; ++column) {
            if (level.data[row * level.columns + column] == look_for) {
                ...
                if (CheckCollisionRecs(player_hitbox, block_hitbox)) {
                    if (level.data[row * level.columns + column] == 'S') {
                        // Шипы наносят урон или убивают игрока
                        //player_lives--; // Например, убавляем жизни
                        return true;
                    }
                    return true;
                }
...
bool is_collodong_with_spikes(Vector2 pos,level &level) {
    return is_colliding(pos, SPIKE, level);
}
